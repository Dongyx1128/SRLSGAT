...
log_interval = 50
model_name = ''
test_path = ''  # '/home/shiyanshi/dyx/SRLSGAT/mcodes/dataset/Chikusei_x4/Chikusei_test.mat'
...
def main():
    ...
    train_parser.add_argument("--batch_size", type=int, default=64, help="batch size, default set to 64")
    train_parser.add_argument("--epochs", type=int, default=70, help="epochs, default set to 70")
    train_parser.add_argument("--out_feats", type=int, default=256, help="hidde_feats, default set to 256")
    train_parser.add_argument("--n_blocks", type=int, default=6, help="n_blocks, default set to 6")
    train_parser.add_argument("--n_subs", type=int, default=31, help="n_subs, default set to 8")
    train_parser.add_argument("--n_ovls", type=int, default=0, help="n_ovls, default set to 0")
    train_parser.add_argument("--n_scale", type=int, default=2, help="n_scale, default set to 2")
    ...
    train_parser.add_argument("--model_title", type=str, default="SRLSGAT", help="model_title, default set to model_title")
    ...
    train_parser.add_argument("--imgsize", type=int, default="64", help="input imgsize (default: 64)")
    ...
    test_parser.add_argument("--out_feats", type=int, default=256, help="hidden_feats, default set to 256")
    test_parser.add_argument("--n_blocks", type=int, default=6, help="n_blocks, default set to 6")
    test_parser.add_argument("--n_subs", type=int, default=31, help="n_subs, default set to 8")
    test_parser.add_argument("--n_ovls", type=int, default=0, help="n_ovls, default set to 1")
    test_parser.add_argument("--n_scale", type=int, default=2, help="n_scale, default set to 2")
    test_parser.add_argument("--use_share", type=bool, default=True, help="f_share, default set to 1")
    test_parser.add_argument("--model_title", type=str, default="SRLSGAT", help="model_title, default set to model_title")
...
def train(args):
    ...
    train_path  = '/home/shiyanshi/dyx/SRLSGAT/mcodes/dataset/' + args.dataset_name + '_x' + str(args.n_scale) + '/trains/'
    test_path   = '/home/shiyanshi/dyx/SRLSGAT/mcodes/dataset/' + args.dataset_name + '_x' + str(args.n_scale) + '/tests/'
    eval_path   = '/home/shiyanshi/dyx/SRLSGAT/mcodes/dataset/' + args.dataset_name + '_x' + str(args.n_scale) + '/evals/'
    result_path = '/home/shiyanshi/dyx/SRLSGAT/mcodes/dataset/' + args.dataset_name + '_x' + str(args.n_scale) + '/pred/'
    ...
    eval_loader = DataLoader(eval_set, batch_size=args.batch_size, num_workers=4, shuffle=False)  # batch_size, input size is ([B, C, H, W]).
    ...
    net = SRLSGAT(n_subs=args.n_subs, n_ovls=args.n_ovls, in_feats=channels, out_feats=args.out_feats, n_blocks=args.n_blocks, n_scale=args.n_scale, use_share=args.use_share, conv=default_conv)
    # print(net)
    model_title = args.dataset_name + "_x" + str(args.n_scale) + "_" + args.model_title + '_Blocks=' + str(args.n_blocks) + '_Feats=' + str(args.out_feats)
    ...
    for e in range(start_epoch, args.epochs):
        ...
        for iteration, (x, lms, gt) in enumerate(train_loader):
            ...
            if (iteration + log_interval) % log_interval == 0:
                print(
                    "===> {} Fea{} GPU{}\tEpoch[{}]({}/{}): Loss: {:.6f}".format(time.ctime(), args.out_feats,
                                                                                           args.gpus, e + 1,
                                                                                           iteration + 1,
                                                                                           len(train_loader),
                                                                                           loss.item()))
...
def test(args):
    ...
    print('===> Start testing')
    # loading model
    model = SRLSGAT(n_subs=args.n_subs, n_ovls=args.n_ovls, in_feats=channels, out_feats=args.out_feats, n_blocks=args.n_blocks, n_scale=args.n_scale, use_share=True, conv=default_conv)
    model_name = "/home/shiyanshi/dyx/SRLSGAT/trained_model/Wdc_x2_SRLSGAT_Blocks=6_Feats=128_epoch_70_Mon_Sep_19_17:05:05_2022.pth"
    with torch.no_grad():
        epoch_meter = meter.AverageValueMeter()
        epoch_meter.reset()
